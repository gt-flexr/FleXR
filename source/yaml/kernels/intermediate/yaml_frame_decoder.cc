#include <yaml/kernels/intermediate/yaml_frame_decoder.h>
      ...
      if(baseSet && specificSet)
      {
        ...
        for(int i = 0; i < inPorts.size(); i++)
        {
          if(inPorts[i].portName == "in_encoded_frame")
          {
            ...
            {
              temp->activateInPortAsLocal<kernels::DecoderInEncodedFrameType>(inPorts[i].portName);
            }
            else if(inPorts[i].connectionType == "remote")
            {
              temp->activateInPortAsRemote<kernels::DecoderInEncodedFrameType>(inPorts[i].portName,
                                                                               inPorts[i].protocol,
        ...
        for(int i = 0; i < outPorts.size(); i++)
        {
          ...
          if(outPorts[i].portName == "out_frame")
          {
            if(outPorts[i].connectionType == "local")
              temp->activateOutPortAsLocal<kernels::DecoderOutFrameType>(outPorts[i].portName);
            else if(outPorts[i].connectionType == "remote")
              temp->activateOutPortAsRemote<kernels::DecoderOutFrameType>(outPorts[i].portName,
                                                                          outPorts[i].protocol,
          ...
          else
          {
            ...
            {
              if(outPorts[i].connectionType == "local")
                temp->duplicateOutPortAsLocal<kernels::DecoderOutFrameType>(outPorts[i].duplicatedFrom,
                                                                             outPorts[i].portName);
              else if(outPorts[i].connectionType == "remote")
                temp->duplicateOutPortAsRemote<kernels::DecoderOutFrameType>(outPorts[i].duplicatedFrom,
                                                                             outPorts[i].portName,
                                                                             outPorts[i].protocol,
                                                                             outPorts[i].connectingAddr,
                                                                             outPorts[i].connectingPortNum);
