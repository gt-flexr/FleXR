      ...
      if(baseSet && specificSet)
      {
        ...
        for(int i = 0; i < inPorts.size(); i++)
        {
          if(inPorts[i].portName == "in_frame")
          {
            ...
            {
              temp->activateInPortAsLocal<kernels::FrameConverterInMsgType>(inPorts[i]);
            }
            else if(inPorts[i].connectionType == "remote")
            {
              temp->activateInPortAsRemote<kernels::FrameConverterInMsgType>(inPorts[i]);
            }
          }
        ...
        for(int i = 0; i < outPorts.size(); i++)
        {
          ...
          if(outPorts[i].portName == "out_frame")
          {
            if(outPorts[i].connectionType == "local")
              temp->activateOutPortAsLocal<kernels::FrameConverterOutMsgType>(outPorts[i]);
            else if(outPorts[i].connectionType == "remote")
              temp->activateOutPortAsRemote<kernels::FrameConverterOutMsgType>(outPorts[i]);
          }
          else
          {
            ...
            {
              if(outPorts[i].connectionType == "local")
                temp->duplicateOutPortAsLocal<kernels::FrameConverterOutMsgType>(outPorts[i]);
              else if(outPorts[i].connectionType == "remote")
                temp->duplicateOutPortAsRemote<kernels::FrameConverterOutMsgType>(outPorts[i]);
            }
