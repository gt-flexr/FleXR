...
int main(int argc, char const *argv[])
{
  ...
  mxre::kernels::MessageReceiver<std::vector<mxre::cv_types::DetectedMarker>> detectedMarkerReceiver(49987, mxre::utils::recvDetectedMarkers);
  ...
  clientDag += detectedMarkerReceiver["out_data"] >> markerCtxExtractor["in_detected_markers"];

  clientDag += imageLoader["out_frame2"] >> objRenderer["in_frame"];
  clientDag += keyboard["out_keystroke"] >> objRenderer["in_keystroke"];
  clientDag += markerCtxExtractor["out_marker_contexts"] >> objRenderer["in_marker_contexts"];
