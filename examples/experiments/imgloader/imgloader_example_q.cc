...
int main(int argc, char const *argv[])
{
  ...
  // cam - obj detector
  pipeline.link(&imageLoader, "out_frame", &orbDetector, "in_frame", QUEUE_SIZE);

  // obj detector - obj ctx extractor
  pipeline.link(&orbDetector, "out_detected_markers", &markerCtxExtractor, "in_detected_markers", QUEUE_SIZE);

  // obj ctx extractor - obj renderer
  pipeline.link(&imageLoader, "out_frame2", &objRenderer, "in_frame", QUEUE_SIZE);
