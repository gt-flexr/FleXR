  ...
  namespace pipeline
  {
    namespace network
    {
      ...
      MatReceiver::MatReceiver(int port, bool isVector) : MessageReceiver(port) {
        ...
        if(isVector)
          output.addPort<std::vector<mxre::cv_units::Mat>>("out_data");
        else
      ...
      raft::kstatus MatReceiver::run() {
        ...
        if(isVector) {
          auto &outData( output["out_data"].allocate<std::vector<mxre::cv_units::Mat>>() );
          ...
          for(uint i = 0; i < vecSize; i++) {
            ...
            mxre::cv_units::Mat newMat(matInfo[MX_MAT_ROWS_IDX], matInfo[MX_MAT_COLS_IDX], matInfo[MX_MAT_TYPE_IDX], inData);
          ...
          outData = mxre::cv_units::Mat(matInfo[MX_MAT_ROWS_IDX], matInfo[MX_MAT_COLS_IDX], matInfo[MX_MAT_TYPE_IDX], inData);
