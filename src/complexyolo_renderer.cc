  ...
  namespace pipeline
  {
    namespace rendering
    {
          ...
          depth(depth), worldMap(0), raft::kernel()
      {
        ...
        input.addPort<std::vector<mxre::perception::complex_yolo::Prediction>>("in_obj_context");
      ...
      raft::kstatus ComplexYOLORenderer::run()
      {
        ...
        auto objCtxVec( input["in_obj_context"].peek<std::vector<mxre::perception::complex_yolo::Prediction>>() );
        ...
        out_frame = mxre::gl::exportGLBufferToCV(width, height); // IMGSIZE!!!!    
