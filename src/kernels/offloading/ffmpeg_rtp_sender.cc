#include "kernels/offloading/ffmpeg_rtp_sender.h"
#include <unistd.h>
...
namespace mxre
{
  namespace kernels
  {
    ...
    /* Constructor() */
    FFmpegRTPSender::FFmpegRTPSender(std::string encoder, std::string destAddr,
    ...
    /* Destructor() */
    FFmpegRTPSender::~FFmpegRTPSender() {
      clearSession();
    ...
    /* setRTPContext() */
    void FFmpegRTPSender::setRTPContext() {
      int ret = 0;
      ...
      rtpContext->max_interleave_delta = 1000;
    ...
    /* setRTPStream() */
    void FFmpegRTPSender::setRTPStreamWithCodec() {
      int ret = 0;
      ...
      rtpCodecContext->thread_count  = 4; // tried 1 - 4. All have same effect.
      rtpCodecContext->thread_type   = FF_THREAD_SLICE;
    ...
    /* setFrameWithScaler() */
    void FFmpegRTPSender::setFrameWithScaler() {
      //uint8_t *rtpFrameBuf;
    ...
    /* sendSDP() */
    void FFmpegRTPSender::sendSDP(std::string &destAddr, int port) {
    ...
    /* clearSession() */
    void FFmpegRTPSender::clearSession() {
    ...
    /* Run() */
    raft::kstatus FFmpegRTPSender::run() {
      ...
      if(gotPkt == 1 && ret == 0 && packet.size) { // check encoding success
        ...
        dataPkt.stream_index = rtpStream->index;
        dataPkt.pts          = packet.pts; // Same as video
        dataPkt.dts          = packet.dts;
